...
First, we see that in the unique case of Gompertz, i.e. $\theta \rightarrow 0$, the dynamics are linear in the log-space and microscopic fluctuations are decoupled from the macroscopic state. This is seen by noting the average macroscopic state reduces to $d\bar{z}/dt = -\beta\bar{z}$, where $\bar{z}$ is the arithmetic weighted mean and we have used the doubly stochastic property of the adjacency matrix, $\frac{1}{N}\sum_i\sum_jw_{ij}z_j=\bar{z}$. Both the second and third terms in Eq. \ref{eq:newMicro} go to zero. Due to the linearity of the Gompertz case, microscopic fluctuations are decoupled from the macroscopic state. 
