...
We now reconcile the SIR model with the Gompertz curve. 
Start with the networked SIR equation for the infected group,
\begin{equation}
\label{eq:SIRnetwork}
...
To build some intuition, we consider some simple scenarios. 
\begin{enumerate}
\item Let $x_j=1$, $\forall j \neq i$. In this scenario, Eq. \ref{eq:modSIR} becomes $\frac{d x_i}{dt} = - \alpha x_i$, implying as expected that the environmental stressor is gone, and the value of $x_i$ goes to zero. In the communicable disease paradigm, Eq. \ref{eq:SIRnetwork} becomes $\frac{d x_i}{dt} = \beta (1-x_i) - \alpha x_i$, and $x_i$ will growth as long as the right-hand side is positive.
\item Let $x_j \rightarrow 0$, $\forall j \neq i$. In this scenario, Eq. \ref{eq:modSIR} $\frac{1}{x_i}\frac{d x_i}{dt} \rightarrow \infty$, implying asymptotically infinite initial growth as the number of higher order terms increases. This behavior is in line with Gompertzian growth and is expected as growth is driven by the susceptible. The implication is that the environmental stressor has just started. In the communicable disease paradigm Eq. \ref{eq:SIRnetwork} becomes $\frac{d x_i}{dt} = - \alpha x_i$, and the value of $x_i$ goes to zero due to the absence of infectious people.
\end{enumerate}
...
\end{equation}
We see that in a mathematically well-defined scenario, nodes in the network respond instantaneously and correlated to each other as under an environmental stressor.
