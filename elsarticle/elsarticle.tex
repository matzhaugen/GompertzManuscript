...
Fourth, we connect the Susceptible-Infected-Recovered (SIR) model with the Gompertz curve by augmenting the logistic growth equation and show that the SIR model is compatible with Gompertzian growth only when all nodes in the transmission network communicate with infinite speed and interaction. We thus find a novel parametrization bridging logistic and Gompertzian growth.
\end{abstract}
...
\section{Simple communicable disease models}
...
\end{equation}
 ...
 Rypdal and Rypdal \cite{Rypdal2020} suggest that the decreasing relative growth rate is caused by social and political mitigating efforts, but these hardly justify such coherent and consistent mortality characteristics across countries, particularly with an exponentially decreasing growth rate. In fact, Wang et al. \cite{wang2012richards} showed that any values of $\theta<1$ are not physically realistic under a similar SIR model as the one given above. 
 ...
 Herrmann and Schwartz \cite{herrmann2020covid} studied a networked SIR model on a variety of networks, but did not elaborate on a possible fit to a Gompertz curve. Estrada and Bartesaghi \cite{estrada2022networked} found that the Gompertz can emerge in later stages of an SIS (Susceptible-Infected-Susceptible) model, but not in the beginning of an SIR-like model.
...
\frac{d}{dt}\ln{x(t)} = - \frac{{}_{1}F_{2}({1,N+1,N+2;1-x(t)})}{N+1}(1-x(t))^{N+1} -\beta \ln{x(t)}.
 \end{equation}
